...
FIRMWARES = {
    "1": {"name": "SSD1306", "keyword": "-ssd"},
    "2": {"name": "SH1106",  "keyword": "-sh"}
}
...
def flash_firmware(bin_path, port):
    ...
    if return_code == 0:
        ...
        print(f"\nError with return code: {return_code}")

def menu(port): 
    print('1. Flash         2. Explorer')
    print('3. Scripts       4. Librates')
    print('5. About         6. Exit')
...
def development(port):
    print("----------------------------------------")
    print("    IN DEVELOPMENT, CHECK FOR UPDATES")
    print("----------------------------------------")
    time.sleep(1)
    menu(port)
...
def about():
    print("-----------------------------------------")
    print(f'    This software need for ')
    print(f'    communicate with ESP-STUN (ESP32)')
    print(f'    specifically for install/ update ')
    print(f'    firmware, download scripts,')
    print(f'    librates, use explorer and all of')
    print(f'    this in one tiny program')
    print("-----------------------------------------")
...
def flash(port):  
    ...
    if not url:
        print("Error getting firmware URL.")
        return
