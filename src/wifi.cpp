...
bool inWiFiConnect = false, wifiScanning = false, wifiSelecting = false, wifiPasswordEntry = false;
int foundNetworks = 0;
String ssidList[10];
...
int wifiConnectIndex = 0;
KeyboardState wifiKeyboard;
String selectedWiFiSSID = "";
wl_status_t wifiConnectStatus = WL_IDLE_STATUS;
...
void handleWiFiConnect() {
  ...
  if (wifiSelecting) {
    ...
    int startIndex = max(0, wifiConnectIndex - 3);
    for (int i = startIndex; i < min(foundNetworks, startIndex + 5); i++) {
      display.setCursor(i == wifiConnectIndex ? 3 : 4, display.getCursorY());
      display.print(i == wifiConnectIndex ? F("> ") : F("  "));
      
      // SSID + type of protection
      String ssidShort = ssidList[i].substring(0, 14);
      display.print(ssidShort);
      
      // display the type of protection
      if (apRecords[i].authmode == WIFI_AUTH_OPEN) {
        display.print(F(" [O]"));
      } else {
        display.print(F(" [*]"));
      }
    ...
    if (buttonUp.isClick()) {
      wifiConnectIndex = max(0, wifiConnectIndex - 1);
    }
    if (buttonDown.isClick()) {
      wifiConnectIndex = min(foundNetworks - 1, wifiConnectIndex + 1);
    }
    if (buttonOK.isClick()) {
      selectedWiFiSSID = ssidList[wifiConnectIndex];
      
      // Если сеть открытая - подключаемся сразу
      if (apRecords[wifiConnectIndex].authmode == WIFI_AUTH_OPEN) {
        display.clearDisplay();
        display.setCursor(3, 20);
        display.println(F("Connecting to:"));
        display.setCursor(3, 32);
        display.println(selectedWiFiSSID);
        display.display();
        
        WiFi.begin(selectedWiFiSSID.c_str());
        wifiConnectStatus = WL_IDLE_STATUS;
        wifiConnectStartTime = millis();
        wifiSelecting = false;
        wifiPasswordEntry = false;
      } else {
        // requestштп password via keybord
        wifiSelecting = false;
        wifiPasswordEntry = true;
        keyboardInit(wifiKeyboard);
      }
    }
    if (buttonBack.isClick()) {
      inWiFiConnect = false;
  ...
  if (wifiPasswordEntry) {
    keyboardRender(display, wifiKeyboard);
    
    
    if (buttonBack.isClick()) {
      wifiPasswordEntry = false;
      wifiSelecting = true;
      wifiKeyboard.buffer[0] = 0;
      wifiKeyboard.bufferPos = 0;
    } else if (buttonOK.isClick()) {
      if (keyboardHandleInput(wifiKeyboard, 4)) {
        if (strlen(wifiKeyboard.buffer) > 0) {
          display.clearDisplay();
          display.setCursor(3, 20);
          display.println(F("Connecting to:"));
          display.setCursor(3, 32);
          ...
          WiFi.begin(selectedWiFiSSID.c_str(), wifiKeyboard.buffer);
          wifiConnectStatus = WL_IDLE_STATUS;
          wifiConnectStartTime = millis();
          wifiPasswordEntry = false;
        } else {
          // Отмена - возвращаемся к выбору
          wifiPasswordEntry = false;
          wifiSelecting = true;
        }
      }
    } else if (buttonOK.isHolded()) {
      // Зажатие OK - быстрое повторение
      keyboardHandleHold(wifiKeyboard);
    } else if (buttonUp.isClick()) {
      keyboardHandleInput(wifiKeyboard, 0);
    } else if (buttonDown.isClick()) {
      keyboardHandleInput(wifiKeyboard, 1);
    }
    return;
  }

  // Режим подключения
  if (!wifiScanning && !wifiSelecting && !wifiPasswordEntry) {
    wifiConnectStatus = WiFi.status();
...
void handleWiFiSubmenu() {
  ...
  if (inWiFiConnect) { handleWiFiConnect(); return; }
  ...
  if (okClick) {
    if (wifiMenuIndex == 0) {
      ...
      WiFi.mode(WIFI_STA);
      WiFi.disconnect();
      WiFi.scanNetworks(true, true);  // asink scan
    }
    if (wifiMenuIndex == 1) {
      inDeauthMenu = true;
      isScanning = true;
